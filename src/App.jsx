    ...
    <div style={{ position: "relative", ...extraStyle }}>
      ...
      {open && (
        <div onClick={e => e.stopPropagation()} style={{
          ...
          padding: "12px", boxShadow: "0 20px 60px rgba(14,165,233,0.25)",
...
function BookingModal({ step, booking, currency = "GBP", onSave, onDelete, onClose, onRename }) {
  ...
  const [form, setForm] = useState({
    ...
    restaurantDate:      booking?.restaurantDate      || "",
    restaurantTime:      booking?.restaurantTime      || "",
    diners:              booking?.diners              || "",
    tourDeparturePoint:  booking?.tourDeparturePoint  || "",
    tourDropOffPoint:    booking?.tourDropOffPoint    || "",
    tourDepartureDate:   booking?.tourDepartureDate   || "",
  ...
  return (
    <div style={overlay} onClick={saveAndClose}>
      <div style={{ ...modal, maxWidth: "500px" }} onClick={e => e.stopPropagation()}>
        ...
        {/* ── Activities / Tours ── */}
        {(step.label.toLowerCase().includes("activit") || (step.label.toLowerCase().includes("tour") && !step.label.toLowerCase().includes("tour package"))) && (<>
        ...
        {/* ── Currency ── */}
        {step.label.toLowerCase().includes("currenc") && (
          <DateField k="pickupDate" label="Collection / Pickup Date" form={form} set={set} />
        )}
        ...
        {/* ── Theme Park ── */}
        {step.label.toLowerCase().includes("theme park") && (<>
        ...
        {/* ── Restaurant ── */}
        {step.label.toLowerCase().includes("restaurant") && (<>
          ...
          <Row>
            <HalfDateField k="restaurantDate" label="Booking Date" form={form} set={set} />
            <HalfTimeField k="restaurantTime" label="Booking Time" form={form} set={set} />
          </Row>
          <HalfField k="diners" label="Number of Diners" placeholder="e.g. 4" form={form} set={set} />
        </>)}

        {/* ── Tour Package ── */}
        {step.label.toLowerCase().includes("tour package") && (<>
          <Row>
            <HalfField k="tourDeparturePoint" label="Departure Point" placeholder="e.g. Manchester Airport" form={form} set={set} />
            <HalfField k="tourDropOffPoint" label="Drop-off Point" placeholder="e.g. Heathrow T5" form={form} set={set} />
          </Row>
          <Row>
            <HalfDateField k="tourDepartureDate" label="Departure Date" form={form} set={set} />
            <HalfField k="tourDuration" label="Duration" placeholder="e.g. 10 nights" form={form} set={set} />
          </Row>
        </>)}

        {/* ── Travel Vaccinations ── */}
        {step.label.toLowerCase().includes("vaccin") && (<>
          <Field k="vaccineName" label="Vaccine Name" placeholder="e.g. Hepatitis A, Yellow Fever" form={form} set={set} />
          <DateField k="vaccinationDate" label="Vaccination Date" form={form} set={set} />
        </>)}
